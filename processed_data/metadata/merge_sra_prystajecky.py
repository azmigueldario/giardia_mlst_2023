...
# for testing
"""
sra_metadata_path = "/project/60006/mdprieto/giardia_mlst_2023/processed_data/metadata/merged_sra_fetchngs_metadata.csv"
...
colnames_drop=['secondary_', 'run_alias', '_title', 'md5', 'fastq_', 'datastore', 'create_date', 'version',
                'biosamplemodel', 'releasedate', 'lat_lon', 'isolation_source']
"""
...
def merge_metadata_giardia(sra_metadata_path, paper_metadata_path, 
    ...
    merged_data = pd.merge(
        sra_data, 
        paper_data, 
        left_on="Library Name", right_on="ubc_id", 
        how="left",
        validate="many_to_one"
        )
    
    # manual from NCBI BioSample descriptions
    merged_data.loc[merged_data['Library Name'].str.contains('ATCC:50163', regex=True, case=False, na=False), ['source', 'province/state,_country']] = ['Cat - ATCC', 'PA,USA']
    merged_data.loc[merged_data['Library Name'].str.contains('ATCC:50803', regex=True, case=False, na=False), ['source', 'province/state,_country']] = ['Strain WB, clone 6 - ATCC', 'MD,USA']
    merged_data.loc[merged_data['Library Name'].str.contains('ATCC:30888', regex=True, case=False, na=False), ['source', 'province/state,_country']] = ['Human - ATCC', 'OR,USA']
    merged_data.loc[merged_data['Library Name'].str.contains('ATCC:50170', regex=True, case=False, na=False), ['source', 'province/state,_country']] = ['Animal - ATCC', 'WI,USA']
    merged_data.loc[merged_data['Library Name'] == 'Giardia beaver', ['source']] = ['Beaver - donated']
    merged_data.loc[merged_data['Library Name'] == 'Giardia BGS', ['source']] = ['Strain GS - ATCC']
    merged_data.loc[merged_data['Library Name'] == 'Giardia AWB', ['source', 'province/state,_country']] = ['Strain WB - ATCC', 'MD,USA']

    # merge isolation source metadata from different columns
    mask = (merged_data['source'] == '') & (merged_data['HOST'] != '')
    merged_data.loc[mask, 'source'] = merged_data.loc[mask, 'HOST']
    
    ####################################################################################
    #               Column Removal 
    ####################################################################################

    # if redundant ID column or sequencing data from reference sample (WB strain) to prevent redundancy
    merged_data = merged_data.drop(columns="ubc_id")
    merged_data = merged_data[~merged_data['Library Name'].str.contains('wb', case=False, na=False)]

    # if specified in list of columns unlikely to be used in downstream analysis
    ...
    # if 30% or more of the data is missing for a column [fill remanining 'NA's with string]
    merged_data = merged_data.dropna(axis=1, thresh=int(0.10 * len(merged_data)))
    merged_data = merged_data.fillna('NA')
    
    # if hybrid assembly (repeated Biosample) keep info for non-Illumina row
        # indicator variable, then sort by Biosample+is_illumina (ascending so False illumina (0) come first)
    merged_data['is_illumina'] = merged_data['Platform'].str.contains('Illumina', case=False, na=True)
    merged_data = merged_data.sort_values(by=['BioSample', 'is_illumina'], ascending=[True, True])
    merged_data = merged_data.drop_duplicates(subset='BioSample', keep='first')
    merged_data = merged_data.drop(columns=['is_illumina'])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "sra_metadata",
        nargs="?",
        default=os.path.join(script_dir, "sraRunTable_2025.csv"),
    ...
    parser.add_argument(
        ...
        nargs='*',
        type=str,
        default=['secondary_', 'run_alias', '_title', 'md5', 'fastq_', 'datastore', 'create_date', 'version',
                 'biosamplemodel'],
